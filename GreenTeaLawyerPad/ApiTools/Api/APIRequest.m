...
//fullUrl
- (NSString *)fullUrl {
    NSString *url = [NSString stringWithFormat:@"%@%@",SERVER_HOST_PRODUCT,self.urlAction];
    return url;
}

- (void)callBackFinishedWithDictionary:(NSDictionary *)dic
{
    [self.delegate serverApi_FinishedSuccessed:self result:dic];
    
    /*
     APIResult *sr = [[APIResult alloc] initWithDictionary:dic];
    if (sr.success) {
        if ([self.delegate respondsToSelector:@selector(serverApi_FinishedSuccessed:result:)]) {
            [self.delegate serverApi_FinishedSuccessed:self result:sr];
        }
    } else {
        if ([self.delegate respondsToSelector:@selector(serverApi_FinishedFailed:result:)]) {
...
{
    DDLogInfo(@"Request FailedError:%@", error);
...
-(NSString*)appVersion
{
    static NSString* appV = nil;
    if(!appV)
    {
        NSDictionary *appInfoDict = [[NSBundle mainBundle] infoDictionary];
        NSString *currentVersion = [appInfoDict objectForKey:@"CFBundleShortVersionString"];
        appV = [NSString stringWithString:currentVersion];
    }
    return appV;
}
